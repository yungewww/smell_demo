...
# PROCESSING = ["diff_data_like"]
# CONFIG_NAME = "long_overlap_first_6"
...
from data_loader import load_sensor_data, build_sliding_data, diff_data_like
from models.transformer import Transformer
from config import DEVICE, DATA_PATH, SAVE_DIR, EPOCHS, LEARNING_RATE, CLASS_LABELS
...
PROCESSING = [
    "diff_data_like"
]  # 想要加 scaler 就写 ["diff_data_like", "standard_scaler"]
# --------------------------------------------------------
...
def train():
    ...
    print(f"Window={WINDOW_SIZE}, Stride={STRIDE}, Channels={CHANNELS}")
    ...
    X, y = build_sliding_data(train_data, WINDOW_SIZE, STRIDE)

    # Select channels
    X = X[:, :, CHANNELS]

    # ------------------------------------------------------------
    # Apply StandardScaler only if requested
    # ------------------------------------------------------------
    scaler = None
    if "standard_scaler" in PROCESSING:
