...
def empty_timeseries_fig():
    ...
    for i, col in enumerate(TARGET_COLUMNS):
        ...
        fig.update_xaxes(title_text="Sample", row=r + 1, col=c + 1)
        fig.update_yaxes(title_text=col, row=r + 1, col=c + 1)
    return fig
...
def preprocess(df: pd.DataFrame):
    ...
    # 取前 6 列（与你训练完全一致）
    df = df.select_dtypes(include=[np.number]).iloc[:, :6].copy()
    if df.empty or len(df) < 80:
        print(f"[WARN] CSV invalid: {df.shape}")
        return None

    # 使用 diff(periods=25)，和 quick_train / original train 一致
